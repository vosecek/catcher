Ext.define("catcher.view.MatchesNavigation", {
    ...
    requires : [ "catcher.view.MatchDetail", "catcher.view.MatchesList", "catcher.view.AddPointDetail", "catcher.view.ScoreList",
            "catcher.view.MatchPlayerList", "catcher.view.EditorPanel", "catcher.view.MainNavigation" ],
    config : {
        title : "Zápasy",
        iconCls : "list",
        items : [ {
    ...
    showInfo : function(show, msg) {
        ...
        if (session.team_filter > 0) {
            store.filterBy(function(record) {
                if (record.get("home_id") == session.team_filter)
                    return true;
                if (record.get("away_id") == session.team_filter)
                    return true;
                return false;
            });
        }
