        ...
        listeners : {
            ...
            painted : function() {
                ...
                if (this.getId() == "MatchPlayerListScore")
                    message = "Vyber skórujícího hráče";
                if (this.getId() == "MatchPlayerListAssist")
                    message = "Vyber nahrávajícího hráče";
                ...
                if (this.getId() == "MatchPlayerListAssist") {
                    if (session.score_player_id > 0) {
                        var skorujici = Ext.getStore("MatchPlayerListAssist").find("player_id", session.score_player_id, false,
                                false, false, true);
            ...
            deactivate : function() {
                var session = getSession();
                var skorujici = Ext.getStore("MatchPlayerListAssist").find("player_id", session.score_player_id, false, false, false,
                        true);
                if (this.getId() == "MatchPlayerListAssist")
                    this.getItemAt(skorujici).hide();
            }
        }
    }
});
