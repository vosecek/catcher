    ...
    config : {
        ...
        toolbar : {
            ...
            items : [
                    ...
                    // target: "",
                    // handler: function(){
                    // Ext.Msg.confirm("Přidat hráče","Tato možnost je pouze pro případ, že hráč není v Evidenci ČALD. Nejprve se
                    // prosím podívej do celkové soupisky týmu na vedlejší ikoně. <br />Opravdu natvrdo přidat hráče?",
                    // function(response){
                    // if(response == "yes"){
                    ...
                    // },
                    {
                        ...
                        handler : function() {
                            Ext.Msg.confirm(
                                "Smazat hráče",
                                "Opravdu natvrdo smazat hráče? Tato možnost je pouze pro případ omylem přidaného hráče přes tuto aplikaci, který není v Evidenci ČALD. Pokud hráč na turnaji nehraje, stačí ho odpojit od týmu v soupisce týmu.",
                                function(response) {
                                    if (response == "yes")
                                        catcher.app.getController("Evidence").deletePlayer()
                                });
                        }
                    } ]
        },
        listeners : {
            ...
            itemtap : function(nested, list, index, target, record) {
                if (record.isLeaf() == false) {
                    ...
                    Ext.getCmp("connectPlayer").target = record.getId();
                    Ext.getCmp("connectPlayer").show();
                    Ext.getCmp("teamList").setBackText("Týmy");
                } else {
                    var shortName = Ext.getStore("Teams").findRecord("team_id", Ext.getCmp("connectPlayer").target);
