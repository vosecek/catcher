...
//        },
    ...
    listeners: {
      leafitemtap: function(nestedList, list, index, target, record){
        if(checkPermission(Ext.getCmp("connectPlayer").target) == true) {
          catcher.app.getController("Evidence").showPlayer(list, record);
        }else{          
          var session = getSession();          
          Ext.Msg.alert("Přístup zamítnut","Tento tým nespadá pod uživatele "+session.get("user")+".<br />Pokud potřebuješ upravit hráče, požádej organizátora.",function(){
...
//         Ext.getCmp("addPlayer").hide();
      ...
      itemtap: function(nested, list, index, target, record){       
        if(record.isLeaf() == false){
          ...
          if(checkPermission(record.getId()) == true) Ext.getCmp("connectPlayer").show();
...
//          addPlayer.show(); // zobrazit, jsme na výpisu týmu, předchozí node byl leaf
        if(checkPermission(connectPlayer.target) == true) connectPlayer.show();
        }else{          
