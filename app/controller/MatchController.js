Ext.define('catcher.controller.MatchController', {
    ...
    confirmMatchDelete : function(el, index, target, record) {
        ...
        Ext.Msg.confirm("Smazat zápas", "Opravdu chceš smazat zápas? <br />" + record.get("home_name_full") + " vs. "
                + record.get("away_name_full"), function(response) {
            if (response == "yes") {
                var store = el.getStore();
                store.remove(record);
                Ext.Viewport.setMasked({
                    xtype : "loadmask",
                    message : "Mažu zápas na serveru"
                });
    ...
    showMatchDetail : function(list, index, target, record, toFinish) {
        if (typeof toFinish == "undefined")
            toFinish = false;
        Ext.getCmp("tournament").getTabBar().hide(); // skrytí hlavní navigace turnaje
        var match = record;
        Ext.getCmp("matchesNavigation").query("button[navigation_only=true]").forEach(function(el) {
            el.hide()
        }); // skrytí filtrovacích tlačítek
    ...
    addPoint : function(list, index, target, record) {
        ...
        var MatchPlayerListScore = Ext.getStore("MatchPlayerListScore").findRecord("player_id", session.score_player_id, false,
                false, false, true).data;
        var MatchPlayerListAssist = Ext.getStore("MatchPlayerListAssist").findRecord("player_id", assist_player_id, false, false,
                false, true).data;
    ...
    updatePoint : function() {
        ...
        Ext.getStore("Points").syncWithListener(
                function() {
                    var controller = catcher.app.getController("MatchController");
                    var matchId = Ext.getStore("Session").findRecord("uuid", Ext.device.Device.uuid).match_id;
                    var scoringPlayer = Ext.getStore("Players").findRecord("player_id", values.scoringPlayer, false, false, false,
                            true).data;
    ...
    fillMatchDetailScore : function(match) {
        ...
        var selects = formular.query("selectfield").forEach(function(el) {
            el.setOptions(cisla);
        });
    ...
    fillMatchDetailSettings : function(match) {
        ...
        formular.setValues(match.data);
        var session = getSession();
        var tournament_data = Ext.getStore("Tournaments").findRecord("tournament_id", session.get("tournament_id"), false, false,
                true);
    ...
    updateMatchScore : function() {
        ...
        if (values.score_home !== undefined) {
            if (values.score_home < match.get("score_home"))
                diffs_fatal.push("Skóre " + match.get("home_name_short") + " je menší než počet uložených bodů ("
                        + match.get("score_home") + "), pro uložení zápasu smaž některé body!");
            if (values.score_home > match.get("score_home"))
                diffs.push("Existující skóre " + match.get("home_name_short") + ": " + match.get("score_home") + ", vkládané: "
                        + values.score_home);

            if (values.score_away < match.get("score_away"))
                diffs_fatal.push("Existující skóre " + match.get("away_name_short") + " je menší než počet uložených bodů ("
                        + match.get("score_away") + "), pro uložení zápasu smaž některé body!");
            if (values.score_away > match.get("score_away"))
                diffs.push("Existující skóre " + match.get("away_name_short") + ": " + match.get("score_away") + ", vkládané: "
                        + values.score_away);
        }

        Ext.Msg.confirm("Zadávaný výsledek", match.get("home_name_short") + " vs. " + match.get("away_name_short") + ": "
                + values.score_home + ":" + values.score_away, function(response) {
            if (response == "yes") {
                if (diffs_fatal.length > 0) {
                    ...
                    if (diffs.length > 0) {
                        Ext.Msg.confirm("Vyšší skóre",
                                "Zadané skóre je vyšší než naskórované, vytvořím anonymní body, opravdu?<br />"
                                        + diffs.join("<br />"), function(response) {
                                    if (response == "yes") {
                                        var points = Ext.getStore("Points");
                                        ...
                                        if (values.score_home > match.get("score_home"))
                                            equalizer((values.score_home - match.get("score_home")), match.get("home_id"));
                                        if (values.score_away > match.get("score_away"))
                                            equalizer((values.score_away - match.get("score_away")), match.get("away_id"));
                                        points.syncWithListener(function() {
                                            var controller = catcher.app.getController("MatchController");
            ...
            if (record.get("finished") == false) {
                ...
                if (record.get("time").getTime() + length + cap < new Date().getTime()) {
                    Ext.Msg.confirm(
                        "Neukončený zápas",
                        "Zápas "
                                + record.get("home_name_short")
                                + " vs. "
                                + record.get("away_name_short")
                                + " není ukončený a zřejmě již skončil. Klikem na Yes přejdeš do detailu zápasu a po zkontrolování výsledného skóre jej ukončíš.",
                        function(response) {
                            if (response == "yes") {
                                catcher.app.getController("MatchController").showMatchDetail(false, false, false,
                                        record, true);
                            }
                        });
                }
...
function getMatchScore(matchId) {
    ...
    points.each(function(item, index, length) {
        ...
        if (item.get("player_id") == 0)
            scoringPlayer = getAnonym();
        if (item.get("assist_player_id") == 2147483647)
            assistPlayer = getCallahan();
        if (item.get("assist_player_id") == 0)
            assistPlayer = getAnonym();
